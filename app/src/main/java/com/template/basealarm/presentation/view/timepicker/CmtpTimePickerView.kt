...
import com.template.basealarm.presentation.view.timepicker.recycler.CmtpValuesAdapter
...
class CmtpTimePickerView @JvmOverloads constructor(
  ...
  private fun setupRecyclersData() {
    val hours = when (time.getType()) {
      CmtpTimeType.HOUR_12 -> CmtpTimeData.HOURS_12
      CmtpTimeType.HOUR_24 -> CmtpTimeData.HOURS_24
    }
    cmtpRecyclerHours.apply {
  ...
  fun getTime24(): CmtpTime24 {
    check(time.getType() == CmtpTimeType.HOUR_24) { "Can't retrieve time in 24-Hour format. TimePicker view was initialised with 12-Hour format." }
  ...
  fun getTime12(): CmtpTime12 {
    check(time.getType() == CmtpTimeType.HOUR_12) { "Can't retrieve time in 12-Hour format. TimePicker view was initialised with 24-Hour format." }
